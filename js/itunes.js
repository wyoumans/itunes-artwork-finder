...
function performSearch() {
  ...
  $.ajax({
        ...
        for (var i = 0; i < data.length; i++) {
          ...
          if (entity != "software") {
            html +=
              '<p><a href="' +
              result.url +
              '" target="_blank">Standard Resolution</a> | <a href="' +
              result.hires +
              '" target="_blank">High Resolution</a> <em><small>' +
              result.warning +
              "</small></em></p>";
          } else {
            html +=
              '<p><a href="./app/?url=' +
              encodeURIComponent(result.appstore) +
              '" target="_blank">View screenshots / videos</a></p>';
          }
          html +=
            '<a href="' +
            result.url +
    ...
    $("#results").append(
      '<p>If the item you are searching for is not available on iTunes, this tool will not find it. Please do not email me asking for specific items if they are not available on iTunes! I recommend both <a href="https://code.google.com/p/subler/">Subler</a> and <a href="https://www.google.co.uk/imghp?gws_rd=ssl">Google Image Search</a> as good alternative places to find artwork.</p>'
    );
  });
}
